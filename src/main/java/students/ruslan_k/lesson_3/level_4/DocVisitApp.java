...
//--------------------------------------------------------------------------------------------------
            ...
            //visit[visitCount].setVisitDate(visitData);

            System.out.println("На эту дату ");
            if (!visit[visitCount].isAmVisit)
                System.out.println("Есть запись " + docVisitHours[0]);
            if (!visit[visitCount].isPmVisit)
...
//--------------------------------------
            ...
            System.out.println("------------------------------");
            System.out.printf("Больной %s %s записан(а) %s к врачу %s %s на часы %s с диагнозом %s \n", visit[0].client.getClientFirstName(), visit[0].client.getClientSecName(), visit[0].getVisitDate(), visit[0].getDocFirstName(), visit[0].getDocSecName(), visitSelect,visit[0].client.getDiagnose());
            System.out.printf("Цена приема: %s \n", visit[0].client.getVisitPrice());
